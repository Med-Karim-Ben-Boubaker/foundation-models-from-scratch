...
def print_generation_step(step: GenerationStep, tokenizer, top_k: int = 5) -> None:
    ...
    # Use topk_probs instead of full probs
    topk_probs = step.topk_probs
    if topk_probs.dim() > 1:
    ...
    # Find the probability of the chosen token in top-k values
    ...
    for i, token_id in enumerate(topk_idx):
        if token_id.item() == step.next_token_id:
            chosen_prob = topk_probs[i].item()
            token_found = True
            break

    if token_found:
        print(f"Chosen probability: {chosen_prob:.4f}")
    else:
        print(f"Chosen probability: <not in top-{len(topk_probs)}>")
    ...
    for i, (prob, token_id) in enumerate(zip(topk_vals, topk_idx)):
        try:
            ...
            print(
                f"  {i + 1}. ID {token_id.item():5d} | P={prob.item():.4f} | '{token}'"
            )
        except:
            print(
                f"  {i + 1}. ID {token_id.item():5d} | P={prob.item():.4f} | <decode_error>"
            )
